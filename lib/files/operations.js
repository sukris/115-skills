...
class FileOperations {
  ...
  async moveFiles(fid, targetCid = '0') {
    ...
    if (!response.state) {
      throw new Error(response.error || '移动文件失败');
    }
  ...
  async copyFiles(fid, targetCid = '0') {
    ...
    if (!response.state) {
      throw new Error(response.error || '复制文件失败');
    }
  ...
  async setStar(fid, star = true) {
    ...
    if (!response.state) {
      throw new Error(response.error || '设置星标失败');
    }
  ...
  async getRecycleBin(options = {}) {
    ...
    if (!response.state) {
      throw new Error(response.error || '获取回收站失败');
    }
  ...
  async restoreFiles(fid) {
    ...
    if (!response.state) {
      throw new Error(response.error || '恢复文件失败');
    }
  ...
  async clearRecycleBin() {
    ...
    if (!response.state) {
      throw new Error(response.error || '清空回收站失败');
    }

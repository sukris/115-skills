...
class FileBrowser {
  ...
  async getFileDetail(fileId) {
    ...
    if (!response.state) {
      throw new Error(response.error || '获取文件详情失败');
    }
  ...
  async getDirectoryPath(cid) {
    ...
    if (!response.state) {
      throw new Error(response.error || '获取目录路径失败');
    }
  ...
  async getCategoryStats(cid = '0') {
    ...
    if (!response.state) {
      throw new Error(response.error || '获取分类统计失败');
    }
  ...
  async getRecentFiles(options = {}) {
    ...
    if (!response.state) {
      throw new Error(response.error || '获取最近文件失败');
    }

...
class Auth115 {
  ...
  async generateQRCode() {
    try {
      const response = await axios.get(`${this.apiBase}/app/qrcode`, {
        params: {
          client: 'web',
          account: '',
          _: Date.now()
  ...
  async checkQRStatus(qrcodeKey) {
    try {
      const response = await axios.get(`${this.apiBase}/app/qrcode/status`, {
      ...
      switch (status) {
        case 2: // 登录成功
          const cookie = this.parseCookie(data.data.cookie);
          return {
            success: true,
            cookie,
            status: 'logged_in',
